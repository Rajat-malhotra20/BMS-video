...
type flvHub struct {
	...
	// onChannelClosed, if set, is called with c.key once every variant of
	// that camera has been torn down (idle past idleGrace, then cancelled)
	// — see release(). This is the deterministic counterpart to an
	// adapter's own capacity accounting (e.g.
	// chemitoapi.Adapter.acquireSlot): a slot reserved when a channel's
	// resolver first ran should be freed when the device session genuinely
	// ends, not on a guess.
	onChannelClosed func(key string)
}
...
func (h *flvHub) release(c *flvChannel) {
	...
	time.AfterFunc(h.idleGrace, func() {
		...
		if h.onChannelClosed != nil {
			// Only once no other quality of this same camera is still open —
			// see onChannelClosed's doc for why a shared per-camera slot must
			// outlive any single variant's teardown.
			stillOpen := false
			for _, other := range h.chans {
				if other.key == c.key {
					stillOpen = true
					break
				}
