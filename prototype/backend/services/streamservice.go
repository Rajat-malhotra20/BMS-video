...
func (s *StreamService) StartStream(ctx context.Context, req domain.StreamRequest) (domain.StreamResult, error) {
	...
	case domain.KindFLV:
		...
		// through our proxy (see flvHub / handleFLVProxy), which calls
		// src.Upstream itself, once per real device connection — not once
		// per viewer and not on a frontend refresh timer. That's what lets
		// the browser hold a single stable URL (/api/flv/{key}) instead of
		// re-fetching /api/stream on any schedule: the vendor's single-use,
		// short-lived token is only ever spent by the hub reconnecting
		// underneath, invisibly to whoever's watching.
		//
...
//
// A no-op for kind: flv beyond dropping the registry entry: the hub (see
// flvhub.go), not this map, owns the real device connection, and it keeps
// serving that connection to any viewer still attached regardless of this
// call. An adapter's capacity slot (vendors.Releaser) is freed by the hub
// itself when it actually tears the channel down — see
// flvHub.onChannelClosed — not from here, since a stop request racing
// ahead of the hub's last viewer leaving must not free a slot that's still
// in real use.
...
type DirectEntry struct {
	...
	// fresh vendor stream URL each time the hub opens (or reopens) the one
	// real device connection for this channel. Never serialized — the
	// browser only ever sees HLSURL (our proxy path), so the vendor's
	// single-use token stays server-side.
