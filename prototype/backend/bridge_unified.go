...
func (u *unifiedBridgeServer) ensureStream(ctx context.Context, bus string, cam int) (*domain.StreamResult, error) {
	...
	// audio=true: the adapter itself now always requests audio from the
	// vendor regardless of this flag (see vendors/chemitoapi.ResolveLiveSource)
	// to sidestep Chemito's FLV header lying about audio when asked for
	// audio=0. Kept true here too so callers reading this StreamRequest see
